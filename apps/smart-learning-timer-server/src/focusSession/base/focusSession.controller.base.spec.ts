...
const service = {
  ...
  focusSessions: () => FIND_MANY_RESULT,
  focusSession: ({ where }: { where: { id: string } }) => {
    switch (where.id) {
      case existingId:
...
describe("FocusSession", () => {
  ...
  test("GET /focusSessions/:id non existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/focusSessions"}/${nonExistingId}`)
  ...
  test("GET /focusSessions/:id existing", async () => {
    await request(app.getHttpServer())
      .get(`${"/focusSessions"}/${existingId}`)
      .expect(HttpStatus.OK)
      .expect({
        ...FIND_ONE_RESULT,
  ...
  test("POST /focusSessions existing resource", async () => {
